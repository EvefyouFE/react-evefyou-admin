...
import { useGlobSetting, useMessage } from '@/hooks';
import { RequestEnum, ResultEnum, ContentTypeEnum } from '@/enums';
import { getToken, deepMerge } from '@/utils';
...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (hasSuccess) {
      ...
      if (options.successMessageMode === 'modal') {
        createSuccessModal({ title: formatById('sys.api.success.tip'), content: successMsg });
    ...
    switch (code) {
      case ResultEnum.TIMEOUT:
        ...
        const [, { setToken, logout }] = useUserRecoilState();
        setToken(undefined);
        logout(true);
        break;
      default:
    ...
    if (options.errorMessageMode === 'modal') {
      createErrorModal({ title: formatById('sys.api.error.tip'), content: timeoutMsg });
  ...
  responseInterceptorsCatch: (axiosInstance: AxiosResponse, error: any) => {
    const { formatById } = useLocale();
    const [, { addAjaxErrorInfo }] = useErrorLogRecoilState();
