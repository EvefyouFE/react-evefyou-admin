...
const transform: AxiosTransform = {
   ...
   */
  transformResponseHook: (res: AxiosResponse<Res>, options: RequestOptions) => {
  ...
  responseInterceptorsCatch: (axiosInstance: AxiosResponse, error: any) => {
    ...
    try {
      if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
        errMessage = formatOutside('sys.api.apiTimeoutMessage');
      }
      if (err?.includes('Network Error')) {
        errMessage = formatOutside('sys.api.networkExceptionMsg');
      }
